...
`);
  ...
  // app/layout.tsx
  const cleanRepoUrl = repoUrl.replace('git+', '').replace('.git', '') || 'https://github.com/';
  fs.mkdirSync(path.join(od, 'app'), { recursive: true });
  writeFile(path.join(od, 'app', 'layout.tsx'), `import { Layout, Navbar } from 'nextra-theme-docs'
...
export default async function RootLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <html lang="en" suppressHydrationWarning>
      ...
      <body>
        <Layout
          navbar={<Navbar logo={<b>${title} Docs</b>} />}
          footer={<p>Generated by doc-generator skill</p>}
          docsRepositoryBase="${cleanRepoUrl}"
          pageMap={pageMap}
        >
          {children}
        </Layout>
...
`);
  ...
  ['getting-started', 'scripts', 'api', 'components'].forEach(dir => {
    fs.mkdirSync(path.join(od, 'content', dir), { recursive: true });
  });
...
`);
  ...
  writeFile(path.join(od, 'content','getting-started', 'installation.mdx'), `# Installation
...
`);
  ...
  writeFile(path.join(od, 'content','getting-started', 'configuration.mdx'), `# Configuration
...
`;
  ...
  writeFile(path.join(od, 'content','scripts', 'index.mdx'), `# Scripts
...
function generateApi(analysis, od) {
  ...
  if (apiRoutes.length === 0) {
    writeFile(path.join(od, 'content','api', 'index.mdx'), `# API Reference\n\nNo API routes were detected in this project.\n`);
    return;
  }
...
`;
    ...
    writeFile(path.join(od, 'content','api', `${slugify(group)}.mdx`), `# /${group}\n\n${content}`);
  });
}
...
function generateComponents(analysis, od, components) {
  if (components.length === 0) {
    writeFile(path.join(od, 'content','components', 'index.mdx'), `# Components\n\nNo React components were detected in this project.\n`);
    return;
  }
...
`);
  ...
  if (!useIndex) {
    components.forEach(comp => {
      ...
      writeFile(path.join(od, 'content','components', `${slugify(comp.name)}.mdx`), `# ${comp.name}
